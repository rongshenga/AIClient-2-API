...
把这个变更拆成 6~7 个主线可并行推进的工作流，并预留 1 个后置收尾工作流，让多个 Codex CLI 先并行完善各自子计划，再统一进入代码实现、集成与迁移收尾。
...
- 目标：补测试、做并发/崩溃一致性验证、大号池性能验证、灰度开关验证，并承担最终集成回归。
- 主范围：`tests/`、文档、验证脚本、必要的 build/test 流程。
- 优先产出：
  - db mode 的 CRUD / 健康状态 / UUID 刷新 / quick link / batch import 测试
  - SQLite CLI 启动开销基准、并发写入压力、进程崩溃恢复与数据一致性验证矩阵
  - 大池选点、批量 flush、分页查询、启动恢复性能验证
  - 特性开关灰度/回退验证矩阵
- 依赖：消费前 1~6 的最终接口；尽量不要反向要求业务层再返工。
- 备注：如果只开 6 个 CLI，可把本工作流并入 CLI-6。

### CLI-8：现有 `configs/` 存量数据迁移收尾与切换执行（后置）

- 目标：在 CLI-1~6 完成并冻结主要接口后，单独处理现有 `configs/` 存量数据的 inventory、baseline backfill、cutover gate、异常处理与最终迁移验收。
- 主范围：迁移编排脚本、snapshot manifest、checksum / anomaly report、cutover runbook、迁移验收文档。
- 优先产出：
  - `configs/` 存量数据清单、checksum 快照、异常文件报告
  - `provider_pools.json`、凭据目录索引、`usage-cache.json`、`token-store.json`、`api-potluck` 数据的基线回填编排
  - `file -> dual-write -> db` 切换闸门、冻结窗口、回滚前置检查
  - 迁移后记录数 / 关键字段 / 兼容投影 diff 的验收报告
- 依赖：消费 CLI-1 的最终存储契约、CLI-4/5 的回填能力、CLI-6 的迁移/导出/回滚工具；必要时与 CLI-7 共享验证矩阵，但默认不回溯修改主线实现。
- 禁止越界：不要重新设计 RuntimeStorage 契约，不要重写 `ProviderPoolManager` / Provider CRUD / usage-plugin 主业务逻辑；如发现阻塞，先记录缺口并回传协调者。
...
- CLI-8 默认不在主线并行阶段启动，等 CLI-1~6 完成并冻结主要接口后再接手存量数据迁移收尾。
...
- CLI-8 建议在 CLI-1~6 完成并冻结主要接口后再启动；如果 CLI-7 已经产出验证矩阵，则直接复用。
- 默认只消费主线已有接口、脚本和验证结论，不回头打断已经运行的 CLI-1~6。
- 如果资源紧张，可等 CLI-7 完成后再执行；如果资源充足，可在 CLI-7 做验证矩阵期间并行准备 runbook，但不要提前改核心实现。
- 合并顺序建议排在 CLI-7 之后，作为迁移验收、切换执行与回滚预案的收尾工作流。
...
- `configs/` 存量数据 inventory / snapshot manifest / cutover runbook / 最终迁移验收默认只让 CLI-8 动。
