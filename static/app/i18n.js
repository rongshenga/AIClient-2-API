...
const translations = {
    'zh-CN': {
        ...
        'tutorial.storage.title': 'Runtime Storage (file/db)',
        'tutorial.storage.desc': '运行时数据存储后端可选 file 或 db。db 模式使用 sqlite，并支持与历史 JSON 文件互导与回滚。',
        'tutorial.storage.table.title': '关键参数',
        'tutorial.storage.backend': '运行时存储后端：file 或 db',
        'tutorial.storage.dbPath': 'SQLite 数据库路径（仅 db 模式使用）',
        'tutorial.storage.dualWrite': '双写模式，便于迁移阶段同时写入 DB 与文件',
        'tutorial.storage.autoImport': '启动时自动导入历史 provider pools 快照到 DB（仅 db 模式）',
        'tutorial.storage.fallback': 'DB 读写异常时回退到文件存储',
